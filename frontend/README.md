...
Slice 12 adds standalone Angular search/list presentation over the accepted REST API.
Detail navigation and AI workflows are deferred to later slices.
...
`npm test` runs 25 focused Vitest/Angular tests with Angular's HTTP test controller.
They cover all filters, request mapping, server-owned defaults/bounds, results,
pagination, loading/empty/errors, retries and request cancellation.
...
validation states, mobile layout and same-origin API requests. It also runs against
