...
| D4 — AI capability invocation | Slice 14 | Resolved by user approval: in-process Spring AI tool calling. The workflow passes the existing `search_catalog` capability adapter to the chat client; no internal MCP client, no `/mcp` self-connection, no new MCP tool and no MCP contract change. See ARCHITECTURE. |
