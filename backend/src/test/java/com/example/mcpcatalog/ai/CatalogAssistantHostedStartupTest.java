...
/**
 * Slice 15 (D7): with the hosted provider selected but its credential absent and the
 * endpoint unreachable, the application must still start and keep serving the accepted
 * REST, MCP and catalog behavior. Only the assistant endpoint may be affected, and no
 * credential may leak into a response.
 */
@Testcontainers
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT,
		properties = { "catalog.ai.provider=bailian", "spring.ai.openai.api-key=",
				"spring.ai.openai.base-url=http://127.0.0.1:1", "spring.ai.openai.chat.model=qwen-plus",
				"catalog.ai.timeout=2s" })
class CatalogAssistantHostedStartupTest {
	...
	@Test
	void startsAndServesTheAcceptedBehaviorInHostedModeWithoutCredentials() throws Exception {
	...
	@Test
	void selectsTheOpenAiCompatibleChatModelForTheHostedProvider() {
		assertThat(this.context.getBean(OpenAiChatModel.class)).isNotNull();
		assertThat(this.context.getBeanNamesForType(OllamaChatModel.class)).isEmpty();
	}
